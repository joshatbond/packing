...
type Item = (typeof mockData)['boxes'][number]['items'][number]
...
export const columns: ColumnDef<Item>[] = [
  ...
  },
  {
    ...
    cell: ({ row }) => (
      // eslint-disable-next-line @typescript-eslint/no-unnecessary-type-assertion
      <Link href={`/images/${row.getValue('image') as string}`}>
  ...
  },
  {
    ...
    filterFn: (row, id, value: string, addMeta) => {
      const itemRank = rankItem(
        // eslint-disable-next-line @typescript-eslint/no-unnecessary-type-assertion
        (row.getValue('labels') as Item['labels']).map(label => label.label),
        value
      )
      addMeta({ itemRank })
